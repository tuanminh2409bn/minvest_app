...
class _WelcomeScreenState extends State<WelcomeScreen> {
  ...
  Widget build(BuildContext context) {
    if (FirebaseAuth.instance.currentUser != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) Navigator.of(context).pushNamedAndRemoveUntil('/', (r) => r.isFirst);
      });
    }
    ...
    return Scaffold(
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final double stackHeight = (650 + (effectiveGap * 2)).clamp(720.0, 1000.0);
          ...
          final content = isNarrow
              ? Padding(
                  ...
                  child: _GlowWrapper(child: _buildFormCard(formPadding)),
                )
              : SizedBox(
                  height: stackHeight,
                  child: Stack(
                    children: [
                      ...
                      Center(
                        child: ConstrainedBox(
                          ...
                          child: _buildFormCard(formPadding),
                        ),
                      ),
                    ],
                  ),
  ...
  Widget _buildFormCard(double verticalPadding) {
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: 16, vertical: verticalPadding.clamp(80.0, 180.0)),
      child: Center(
        child: ConstrainedBox(
          constraints: const BoxConstraints(maxWidth: 520),
          child: Container(
...
class _HoverCardState extends State<_HoverCard> {
  ...
  @override
  Widget build(BuildContext context) {
    final double hoverOffset = _isHover ? (widget.hoverDirectionUp ? -6 : 6) : 0;
    return MouseRegion(
